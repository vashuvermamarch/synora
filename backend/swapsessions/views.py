...
def create_session(request):
    ...
    session = serializer.save(status='accepted')
...
@api_view(['PATCH'])
@permission_classes([IsAuthenticated])
def respond_swap_request(request, pk):
    """Accept or decline a swap request."""
    try:
        sr = SwapRequest.objects.get(pk=pk, receiver=request.user)
    except SwapRequest.DoesNotExist:
    ...
    sr.status = status_val
    sr.save()

    # Notify Sender
    Notification.objects.create(
        user=sr.sender,
        title=f"Swap Request {status_val.capitalize()}",
        message=f"{request.user.username} has {status_val} your swap request.",
        type='request'
      )

    return Response(SwapRequestSerializer(sr).data)
