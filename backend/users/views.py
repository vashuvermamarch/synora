...
from .models import Profile, VerificationRequest, ContactRequest
...
def contact_us(request):
    """Handle support requests from the Contact Us page."""
    data = request.data
    full_name = data.get('full_name')
    email = data.get('email')
    problem_description = data.get('problem_description')

    if not full_name or not email or not problem_description:
    ...
    return Response({'message': 'Support request received.'}, status=status.HTTP_201_CREATED)
